...
export default class ValidationService {
  ...
  private confirmPassword = yup.string().min(8, `Confirm Password ${minPasswordError}`).oneOf([yup.ref('password'), null], 'Passwords must match').required('Confirm Password is required');
  
  public registerSchema = yup.object({
